...
void ZListCtrl::OnEditKillFocus(void)
{
	...
	m_editModify.GetWindowText(strNew);
	m_editModify.ShowWindow(SW_HIDE);
	strOld = GetItemText(m_ii.m_nItem, m_ii.m_nSubItem);
	SetItemText(m_ii.m_nItem, m_ii.m_nSubItem, strNew);
	if (m_p_callback)
		m_p_callback->OnModifyItem(m_ii, strOld, strNew);
}
